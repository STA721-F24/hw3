...
\item Show that that $\E[\P_\V \eps \eps^T (\I_n - \P_\V)^T ] = \zero$ always for $\Cov[\eps] = \sigma \V$
\item Show that that $\E[\P_{\Phib} \eps, \eps^T(\I_n - \P_{\Phib})^T] = \zero$  under the conditions of the theorem.
\item Show that if $\Cov[\eps] = \sigma^2 \I_n$ that $\E[\P_{\V} \eps \eps^T(\I_n - \P_{\V_n}^T] \ne \zero$ for general $\V$.  (show for $\V$ diagonal, then consider what happens using the spectral decomposition for a general $\V$).
\end{enumerate}
\end{enumerate}
\end{document}
