...
show that by writing the loss function $\| \Y - \X\b\|^2_{\V^{-1}}$ as $\| (\Y - \P_\V\y) + (\P_V \Y - \X\b)\|^2_{\V^{-1}}$ and expanding appropriately that the cross-product term is zero and that $\P_\V \Y$ minimizes the  generalized loss and is the GLS estimator of $\mub$.  Aren the conditions equivalent to $\langle \P_{\Phib}\y,  (\I_n - \P_{\Phib})\y \rangle_{\V^{-1}} = 0 \quad \forall \y \in \bbR^n$$
...
\item (Challenging) Show that for $\Y=\X\b + \eps$ and 
