...
$$\V \equiv \X \Psib \X^T + \Phib$$. 
(Review the steps of the proof in Proposition 2.7.5 in Christensen and the outline in class of the Theorem that showed
...
$\Psib$ and $\Phib$ of the appropriate dimension)


\item Show that that $\E[\P_\V \eps, \eps^T (\I_n - \P_\V)^T ] = \zero$.  (Hint construct an appropriate decomposition of $\eps = \eps_\X + \eps_N$ that has the desired covariance stucture.
\item Show that that $\E[\P_{\Phib} \eps, \eps^T(\I_n - \P_{\Phib})^T] = \zero$ iff $\V = \X\Psib \X^T + \Phib$ (i.e.  $\bhat_\V = \bhat_{\Phib}$ iff $\P_{\Phib} \eps$ and $(\I_n - \P_{\Phib}) \eps$  are uncorrelated.  )
\item Show that if $\Cov[\eps] = \sigma^2 \I_n$ that $\E[\P_{\I_n} \eps \eps^T(\I_n - \P_{\I_n}^T] \ne \zero$, where $\P_{\I_n} \equiv  \X (\XtX)^{-1} \X^T$.
\end{enumerate}
\end{enumerate}
\end{document}
